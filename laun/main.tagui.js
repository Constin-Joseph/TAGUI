...
var automation_start_time = Date.now(); dummy_echo('' + Date().toLocaleString());
...
var quiet_mode = true; var save_text_count = 0; var snap_image_count = 0;
...
var flow_path = 'C:/taguiapps/laun';
...
var home_path = 'C:/Users/ELCOT-Lenovo';

var desktop_path = 'C:/Users/ELCOT-Lenovo/Desktop';

var temp_path = 'C:/Users/ELCOT-~1/AppData/Local/Temp';
...
casper.then(function() {{techo('py GetFilesInFolder(\'C:\\\\taguiapps\\\\hhh\')');
py_result = ''; if (!py_step('py GetFilesInFolder(\'C:\\\\taguiapps\\\\hhh\')'))
...
casper.then(function() {for (index = 0; index < files.length;index++)
{casper.then(function() {for_loop_signal = '[CONTINUE_SIGNAL][index]';});
(function (index) { // start of IIFE pattern
{ // start of code block
...
casper.then(function() {{techo('vision app = App.open(file)'); var fs = require('fs');
if (!sikuli_step('vision app = App.open(file)')) if (!fs.exists('for vision step'))
...
casper.then(function() {{techo('vision ocr_text = Region(109,268,387,56).text()'); var fs = require('fs');
if (!sikuli_step('vision ocr_text = Region(109,268,387,56).text()')) if (!fs.exists('for vision step'))
this.echo('ERROR - cannot find image file for vision step').exit(); else
this.echo('ERROR - cannot find for vision step on screen').exit(); this.wait(0);}});

casper.then(function() {{techo('vision tagui_text = open(\'tagui.sikuli/tagui_sikuli.txt\',\'w\')'); var fs = require('fs');
if (!sikuli_step('vision tagui_text = open(\'tagui.sikuli/tagui_sikuli.txt\',\'w\')')) if (!fs.exists('for vision step'))
this.echo('ERROR - cannot find image file for vision step').exit(); else
this.echo('ERROR - cannot find for vision step on screen').exit(); this.wait(0);}});

casper.then(function() {{techo('vision tagui_text.write(ocr_text.decode(\'utf8\')); tagui_text.close()'); var fs = require('fs');
if (!sikuli_step('vision tagui_text.write(ocr_text.decode(\'utf8\')); tagui_text.close()')) if (!fs.exists('for vision step'))
this.echo('ERROR - cannot find image file for vision step').exit(); else
this.echo('ERROR - cannot find for vision step on screen').exit(); this.wait(0);}});
...
casper.then(function() {{techo('vision ocr_text = Region(765,453,245,25).text()'); var fs = require('fs');
if (!sikuli_step('vision ocr_text = Region(765,453,245,25).text()')) if (!fs.exists('for vision step'))
this.echo('ERROR - cannot find image file for vision step').exit(); else
this.echo('ERROR - cannot find for vision step on screen').exit(); this.wait(0);}});
...
casper.then(function() {{techo('vision app.close()'); var fs = require('fs');
if (!sikuli_step('vision app.close()')) if (!fs.exists('for vision step'))
this.echo('ERROR - cannot find image file for vision step').exit(); else
this.echo('ERROR - cannot find for vision step on screen').exit(); this.wait(0);}});

///>
//< [Click] comment="" value="(1301,5);coordinat"
casper.then(function() {{techo('click (1301,5)'); var fs = require('fs');
if (!sikuli_step('click (1301,5)')) if (!fs.exists('(1301,5)'))
this.echo('ERROR - cannot find image file (1301,5)').exit(); else
this.echo('ERROR - cannot find (1301,5) on screen').exit(); this.wait(0);}});
...
if (download_path.indexOf(':')>0) download_path = download_path.replace(/\//g,'\\').replace(/\\/g,'\\');
chrome_step('Page.setDownloadBehavior',{behavior: 'allow', downloadPath: download_path});
chrome_step('Page.navigate',{url: 'https://form.jotform.com/201242974804454'}); sleep(1000);
techo('https://form.jotform.com/201242974804454' + ' - ' + chrome.getTitle());});

///>
//< [Type] comment="" value="//*[@id="input_3"];input" as="'+name+';variable"
casper.then(function() {{techo('type //*[@id="input_3"] as '+name+'');
casper.waitFor(function check() {return check_tx('//*[@id="input_3"]');},
function then() {chrome.sendKeys(tx('//*[@id="input_3"]'),''+name+'');},
function timeout() {this.echo('ERROR - cannot find //*[@id="input_3"]').exit();});}});

///>
//< [Type] comment="" value="//*[@id="input_4"];input" as="'+invoiceid+';variable"
casper.then(function() {{techo('type //*[@id="input_4"] as '+invoiceid+'');
casper.waitFor(function check() {return check_tx('//*[@id="input_4"]');},
function then() {chrome.sendKeys(tx('//*[@id="input_4"]'),''+invoiceid+'');},
function timeout() {this.echo('ERROR - cannot find //*[@id="input_4"]').exit();});}});

///>
//< [Click] comment="" value="(684,363);coordinat"
casper.then(function() {{techo('click (684,363)'); var fs = require('fs');
if (!sikuli_step('click (684,363)')) if (!fs.exists('(684,363)'))
this.echo('ERROR - cannot find image file (684,363)').exit(); else
this.echo('ERROR - cannot find (684,363) on screen').exit(); this.wait(0);}});
...
casper.then(function() {{techo('click (778,166)'); var fs = require('fs');
if (!sikuli_step('click (778,166)')) if (!fs.exists('(778,166)'))
this.echo('ERROR - cannot find image file (778,166)').exit(); else
this.echo('ERROR - cannot find (778,166) on screen').exit(); this.wait(0);}});
...
})(index); // end of IIFE pattern, with dummy marker for break step
casper.then(function() {for_loop_signal = '[BREAK_SIGNAL][index]';});}});
